...
const checkIsValidUser = (req, res, next) => {
  ...
  if (
    !validator.isNumeric(mobile.toString()) ||
    mobile.toString().length < 10
  ) {
    return res
      .status(400)
      .json({ successful: false, message: "Number is not valid" });
  }
  ...
  if (
    ...
    return res
      ...
      .json({
        successful: false,
        message: "Full name is required and should be valid",
      });
  }
  // if (password.length < 5) {
  //     return res.status(400).json({ successful: false, message: `Password min length is 5` });
  // }
  req.userFullName = `${firstName} ${lastName}`;
  next();
};
...
const checkIsValidUpdate = (req, res, next) => {
  ...
  if (
    ...
    streetNumber.toString().length > 3 ||
    !pinCode ||
    ...
    return res
      ...
      .json({
        successful: false,
        message: "Full address is required and not valid",
      });
  }
  next();
};
